        ...
        <el-col :span="24" :xs="24">
           ...
           <el-table v-loading="loading" :data="TaskInstanceList" @selection-change="handleSelectionChange">
              ...
              <el-table-column label="操作" align="center" width="150" class-name="small-padding fixed-width">
                 <template #default="scope">
                    <el-tooltip content="修改" placement="top" v-if="scope.row.TaskInstanceId !== 1">
                       <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                          v-hasPermi="['system:TaskInstance:edit']"></el-button>
                    </el-tooltip>
                    <el-tooltip content="删除" placement="top" v-if="scope.row.TaskInstanceId !== 1">
                       <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                          v-hasPermi="['system:TaskInstance:remove']"></el-button>
                    </el-tooltip>
                 </template>
...
import {
  ...
  addTaskInstance,
  changStatusField
} from "@/api/data/scheduler/taskInstance";
...
function getList() {
  loading.value = true;
  listTaskInstance(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
     loading.value = false;
     TaskInstanceList.value = res.rows;
...
function handleDelete(row) {
  const TaskInstanceIds = row.id || ids.value;
  proxy.$modal.confirm('是否确认删除应用编号为"' + TaskInstanceIds + '"的数据项？').then(function () {
     return delTaskInstance(TaskInstanceIds);
  }).then(() => {
     getList();
...
function handleUpdate(row) {
  reset();
  const TaskInstanceId = row.id || ids.value;
  getTaskInstance(TaskInstanceId).then(response => {
...
/** 查看项目告警空间 */
function handleTaskInstanceSpace(id){
...
function submitForm() {
  proxy.$refs["databaseRef"].validate(valid => {
     if (valid) {
        if (form.value.TaskInstanceId != undefined) {
