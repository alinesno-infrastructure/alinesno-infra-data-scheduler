<template>
          ...
          <el-col :span="24" :xs="24">
             ...
             <el-table v-loading="loading" :data="EnvironmentList" @selection-change="handleSelectionChange">
                ...
                <el-table-column label="应用名称" align="left" width="250" key="projectName" prop="projectName" v-if="columns[0].visible">
                   <template #default="scope">
                       <el-button type="danger" bg text @click="handleEnvironmentSpace(scope.row.id)"> 
  ...
  function handleDelete(row) {
    ...
    proxy.$modal.confirm('是否确认删除应用编号为"' + EnvironmentIds + '"的数据项？').then(function () {
  ...
  function submitForm() {
    proxy.$refs["databaseRef"].validate(valid => {
       if (valid) {
          if (form.value.EnvironmentId != undefined) {
