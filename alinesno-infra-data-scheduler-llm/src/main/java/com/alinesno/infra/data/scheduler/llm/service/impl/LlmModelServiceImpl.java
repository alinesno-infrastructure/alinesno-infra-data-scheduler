package com.alinesno.infra.data.scheduler.llm.service.impl;
...
import com.alinesno.infra.data.scheduler.llm.event.StreamMessagePublisher;
import com.alinesno.infra.data.scheduler.llm.mapper.LlmModelMapper;
...
//        role.setId(PublishConst.ANONYMOUS_USER);
        ...
        if(modelType.equals(ModelTypeEnums.LARGE_LANGUAGE_MODEL.getCode())){  // 大语言模型
           ...
           return validateLargeLanguageModel(url, apiKey, secretKey , modelName, modelProvider, workflowId);
            ...
            validateVectorModel(url, apiKey, modelName, modelProvider, workflowId);

        } else if(modelType.equals(ModelTypeEnums.RE_RANKING_MODEL.getCode())){  // 重排模型

            validateRankingModel(url, apiKey, modelName, modelProvider, workflowId);

        }else if(modelType.equals(ModelTypeEnums.SPEECH_RECOGNITION.getCode())){  // 语音识别

            return validateSpeechRecognition(url, apiKey, modelName, modelProvider, dto);

        }else if(modelType.equals(ModelTypeEnums.SPEECH_SYNTHESIS.getCode())){  // 语音合成

          return validateSpeechSynthesis(url, apiKey, modelName, modelProvider, workflowId , dto.getVoice());

        }else if(modelType.equals(ModelTypeEnums.IMAGE_GENERATION.getCode())){ // 图片生成

           return validateImageGeneration(url, apiKey, secretKey , modelName, modelProvider, workflowId);

        }else if(modelType.equals(ModelTypeEnums.VISION_MODEL.getCode())){  // 视觉模型

            validateVisionModel(url, apiKey, modelName, modelProvider, workflowId);

        }else if(modelType.equals(ModelTypeEnums.OCR_MODEL.getCode())){  // OCR模型识别
            validateOCRModel(url, apiKey, modelName, modelProvider, workflowId);
        }
    ...
    private void validateOCRModel(String url,
        ...
        streamMessagePublisher.doStuffAndPublishAnEvent(response.getResults(), IdUtil.getSnowflakeNextId()) ;
